...
<div><div>Related Topics</div><div><li>String</li><li>Depth-First Search</li><li>Breadth-First Search</li><li>Interactive</li></div></div><br><div><li>👍 224</li><li>👎 268</li></div>
*/
...
class Solution {
    ...
    public List<String> crawl(String startUrl, HtmlParser htmlParser) {
        ...
        isVisited.add(startUrl);
        dfs(hostname(startUrl), startUrl, htmlParser);
        List<String> url = new ArrayList<>(isVisited);
        return url;
    }
    private String hostname(String url) {
        return url.split("/")[2];
    }
    private void dfs(String hostname, String url, HtmlParser htmlParser) {
        List<String> list = htmlParser.getUrls(url);
        for (String currUrl: list) {
            if (!isVisited.contains(currUrl) && currUrl.contains(hostname)) {
                isVisited.add(currUrl);
                dfs(hostname, currUrl, htmlParser);
            }
        }
    }
}
